...
import { fmt, MONTHS, daysUntilDue, vencStr, fmtM } from "../utils";
...
export function Relatorios({ t, presence, students, plans, schedules, payments = [] }: RelatoriosProps) {
  const [tab, setTab] = useState<"presenca" | "vencimentos">("presenca");
  const [relMonth, setRelMonth] = useState(new Date().toISOString().slice(0, 7));
  ...
  return (
    <div className="space-y-4">
      ...
      <TabSwitcher tabs={[["presenca", "👥 Presença"], ["vencimentos", "🔔 Vencimentos"]]} active={tab} onChange={(k) => setTab(k as "presenca" | "vencimentos")} t={t} />
              ...
              <Card key={s.id} t={t} className={urgente ? "border-amber-300" : ""}>
                ...
                {urgente && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="flex items-center justify-center gap-1 mt-2 w-full py-1.5 text-xs rounded-xl font-medium text-white transition-all"
...
function RelResumo({ presence, activeStudents, plans, relMonth, setRelMonth, setRelStudent, gP, t }: RelResumoProps) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2">
        ...
        <div className="flex-1">
          <label className="text-xs text-gray-400 block mb-1">Aluno</label>
          <select
            onChange={(e) => {
              if (e.target.value !== "todos") setRelStudent(e.target.value);
            }}
      ...
      {byStudent.length === 0 && <p className="text-sm text-gray-400 text-center py-6">Nenhum registro para este mês.</p>}
        ...
        return (
          <Card key={s.id} t={t}>
            <div className="flex items-center gap-3 mb-3">
              ...
              <div className="text-right">
                <p className="text-lg font-bold" style={{ color: bc }}>
                  {pct}%
                </p>
            ...
            <button onClick={() => setRelStudent(s.id)} className="mt-2 text-xs transition-all" style={{ color: t.p[500] }}>
              Ver detalhes →
            </button>
          </Card>
        );
...
function RelDetalhe({ studentId, presence, students, plans, schedules, setRelStudent, gP, gSc, t }: RelDetalheProps) {
    ...
    <div className="space-y-3">
      ...
      <Card t={t}>
        <div className="flex items-center gap-3 mb-3">
          <Av name={s.name} t={t} />
          <div>
            <p className="font-bold" style={{ color: t.p[800] }}>
              {s.name}
            </p>
        ...
        <div className="grid grid-cols-4 gap-2 mb-3">
          {[["Presenças", pres, "#16a34a"], ["Faltas", falt, "#ef4444"], ["Repos.", rep, "#f59e0b"], ["Frequência", `${pct}%`, bc]].map(([l, v, c]) => (
            <div key={l} className="text-center p-2 rounded-xl" style={{ background: t.p[50] }}>
        ...
        return (
          <div key={h.id} className="flex items-center justify-between p-3 rounded-xl mb-1 border" style={{ borderColor: "#e5e7eb" }}>
            <div className="flex items-center gap-3">
              <span>{icon}</span>
              <div>
                <p className="text-sm font-medium" style={{ color: t.p[800] }}>
                  {fmt(h.day)}
                </p>
